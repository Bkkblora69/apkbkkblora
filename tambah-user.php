...
if(isset($_POST['submit'])):			
	...
	if(empty($errors)):
		$pass = sha1($password);
		$simpan = mysqli_query($koneksi,"INSERT INTO user (id_user, username, password, nama, nomor_telepon, role,kode_cabang) VALUES ('', '$username', '$pass', '$nama', '$nomor_telepon', '$role','$kode_cabang')");
		if($simpan) {
		...
		<div class="card-body">
			<div class="row">
				...
				<div class="form-group col-md-6">
					...
					<input autocomplete="off" type="nomor_telepon" name="nomor_telepon" required class="form-control"/>
				</div>

				<div class="form-group col-md-6">
					<label class="font-weight-bold">Kode Cabang</label>
					<input autocomplete="off" type="text" name="kode_cabang" required class="form-control"/>
				</div>
