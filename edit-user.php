...
if(isset($_POST['submit'])):
	...
	if(empty($errors)):
		$update = mysqli_query($koneksi,"UPDATE user SET nama = '$nama', nomor_telepon = '$nomor_telepon', role = '$role', kode_cabang='$kode_cabang' WHERE id_user = '$id_user'");
		
		if($password) {
			$pass = sha1($password);
			$update = mysqli_query($koneksi,"UPDATE user SET nama = '$nama',  password = '$pass', nomor_telepon = '$nomor_telepon', role = '$role' WHERE id_user = '$id_user'");
		}		
		if($update) {
		...
		<div class="card-body">
			<div class="row">
				...
				<div class="form-group col-md-6">
					...
					<input autocomplete="off" type="nomor_telepon" name="nomor_telepon" required value="<?php echo $d['nomor_telepon']; ?>" class="form-control"/>
				</div>
				
				<div class="form-group col-md-6">
					<label class="font-weight-bold">Kode Cabang</label>
					<input autocomplete="off" type="text" name="kode_cabang" value="<?php echo $d['kode_cabang']; ?>" required class="form-control"/>
				</div>
				...
				<div class="form-group col-md-6">
					...
					<select name="role" required class="form-control">
						...
						<option value="user" <?php if($d['role']=="user") {echo "selected";} ?>>User</option>
					</select>
				</div>
			</div>
		</div>
